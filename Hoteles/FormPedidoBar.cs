...
namespace Hoteles
{
    public partial class FormPedidoBar : Form
    {
            ...
            if ((char)Keys.Enter == e.KeyChar)
            {
                ...
                switch (pasoAsignacion)
                {
                    case "nroHabitacion":
                        ...
                        {
                            dgvOpcionesElegidas.Rows.Add(dr[0].ToString() + " " + dr[1].ToString() + "  -  " +String.Format("{0:C}",decimal.Parse(dr[2].ToString())));
                        }                        
                    ...
                    case "cantidad":
                        ...
                        dgvOpcionesElegidas.Rows[indice].Cells[0].Value = cantidad + " " + dgvOpcionesElegidas.Rows[indice].Cells[0].Value + "  -  " + String.Format("{0:C}", DictArticulos[nroArtElegido].precio * cantidad);
                        dgvOpcionesElegidas.ClearSelection();
                        labelNroHab.Text = "Ingresar Nro.Art ";
                        tbNroHab.Text = "0";
                        pasoAsignacion = "articulo";
        ...
        {            
            ...
            dgvPromos.RowTemplate.Height = 49;// con un heigt de 60 entran 6
            dgvOpcionesElegidas.RowTemplate.Height = 49;

            foreach (DataRow dr in articulos)
            {
                dgvPromos.Rows.Add(dr[0], dr[1], dr[2], dr[3]);
                DictArticulos.Add(Convert.ToInt32(dr[0]), new Articulo(Convert.ToInt32(dr[0]), dr[1].ToString(), Convert.ToDecimal(dr[2])));
            }
